...
import { EditableCell } from "@/shared/ui/EditableCell/EditableCell.js";
...
describe("EditableCell", () => {
  ...
  it("shows the rejection reason and restores the stored value", async () => {
    const onSave = () => Promise.reject(new Error("Введите число"));
    render(<Harness onSave={onSave} />);
  ...
  it("reopens a failed cell with the text last typed and clears the error", async () => {
    const onSave = vi.fn(() => Promise.reject(new Error("Введите число")));
    render(<Harness onSave={onSave} />);
