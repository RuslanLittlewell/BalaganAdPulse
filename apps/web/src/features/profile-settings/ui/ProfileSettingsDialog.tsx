...
interface Fields { name: string; currentPassword: string; newPassword: string; }
...
export function ProfileSettingsDialog({ open, onClose, onAvatarSaved }: Props) {
  ...
  const { register, reset, handleSubmit, formState: { isSubmitting, errors } } = useForm<Fields>({ defaultValues: { name: user?.name ?? "", currentPassword: "", newPassword: "" } });
  ...
  useEffect(() => {
    ...
    void loadProfile().then((profile) => {
      reset({ name: profile.name, currentPassword: "", newPassword: "" });
      setOptions(parseAvatarPath(profile.avatarPath));
    }).catch(() => setMessage(t("profile.loadFailed")));
  }, [open, loadProfile, reset]);

  const submit = handleSubmit(async ({ name, currentPassword, newPassword }) => {
    setMessage("");
    try {
      await updateProfile({ name, ...(newPassword ? { currentPassword, newPassword } : {}) });
      reset({ name, currentPassword: "", newPassword: "" });
