...
import { buildDefaultProperties } from "../../src/modules/campaigns/index.js";
import { collectPropertyRefs } from "../../src/modules/campaigns/index.js";
import { expressionSchema } from "../../src/modules/campaigns/presentation/http/campaign-schemas.js";

describe("buildDefaultProperties", () => {
  const properties = buildDefaultProperties(() => randomUUID());
  ...
  it("produces fresh ids on every call", () => {
    expect(buildDefaultProperties(() => randomUUID())[0].id).not.toBe(properties[0].id);
  });
});
