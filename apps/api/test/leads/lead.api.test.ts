...
it('names client boards by the client name, not its organization', async () => {
  ...
  expect(boards.map((b:{key:string;label:string})=>[b.key,b.label])).toEqual([['agency','Agency'],[clientId,'Ромашка']]);
});
...
it('assigns a reachable client member and returns project and assignee details', async () => {
  const teammate=await signInAs('Мария',{role:'MANAGER'});
  const project=await prisma.project.findFirstOrThrow({where:{clientId}});
  await grantAccess(teammate.membership!.id,clientId);
  const response=await request(app).post(url()).set(auth).send({
    name:'Assigned',projectId:project.id,assigneeId:teammate.membership!.id,
  });
  expect(response.status).toBe(201);
  expect(response.body).toMatchObject({
    project:{id:project.id,clientId,name:project.name},
    assignee:{id:teammate.membership!.id,name:'Мария'},
    assigneeId:teammate.membership!.id,
  });
});
it('rejects an assignee who does not belong to the lead client', async () => {
  const teammate=await signInAs('Без доступа',{role:'MANAGER'});
  const project=await prisma.project.findFirstOrThrow({where:{clientId}});
  const response=await request(app).post(url()).set(auth).send({
    name:'Assigned',projectId:project.id,assigneeId:teammate.membership!.id,
  });
...
it.each(['CLIENT','CLIENT_ADMIN'] as const)('allows %s to manage only their own funnel', async role => {
  ...
  expect((await request(app).patch(`${url(clientId)}/${r.body.id}/move`).set(customer.auth).send({stage:'PROPOSAL',position:0})).status).toBe(200);
