...
- [x] 4.3 Run production API and web builds, then `npm test` and `npm run test:web` until both are green.

## 5. Exceeded KPIs

- [x] 5.1 Write and observe failing tests for the exceeded state in the progress model (strictly better for both directions, equal as met), and for the tile writing Выполнено+ when exceeded, Выполнено when met exactly, and no status when behind or not measurable.
- [x] 5.2 Implement the exceeded state, the two status texts and an animated progress bar while exceeded that respects reduced motion; drop the unused status copy.
- [x] 5.3 Run `npm test` and `npm run test:web` until both are green.
