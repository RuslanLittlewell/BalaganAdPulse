...
import { CampaignSheet } from "@/widgets/campaign-sheet/CampaignSheet.js";
...
describe("CampaignSheet", () => {
  it("renders a column per property, a row per record and the totals row", async () => {
    ...
    expect(screen.getByRole("rowheader", { name: "01 авг." })).toBeInTheDocument();
    ...
    expect(within(footer).getByRole("rowheader")).toHaveTextContent("ИТОГО");
  ...
  it("renders the columns and the header even when the sheet has no days", async () => {
    ...
    expect(await screen.findByRole("columnheader", { name: "SPEND" })).toBeInTheDocument();
    expect(screen.getByText("Статистика по дням")).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "+ Добавить день" })).toBeInTheDocument();
  ...
  it("adds the day after the last row", async () => {
    ...
    await userEvent.click(await screen.findByRole("button", { name: "+ Добавить день" }));
  ...
  it("opens an entered cell and leaves a computed one alone", async () => {
    ...
    expect(screen.getByRole("textbox", { name: "SPEND, 01 авг." })).toHaveValue("120");
  ...
  it("clears a cell when the input is emptied", async () => {
    ...
    await userEvent.click(await screen.findByRole("button", { name: "120.00" }));
    await userEvent.clear(screen.getByRole("textbox", { name: "SPEND, 01 авг." }));
  ...
  it("rejects input a numeric column cannot store without calling the API", async () => {
    ...
    expect(cell).toHaveAttribute("title", "Введите число");
    expect(puts).toBe(0);
  });
  ...
  it("tabs from the last entered column of a row into the next row", async () => {
    ...
    expect(screen.getByRole("textbox", { name: "SPEND, 02 авг." })).toBeInTheDocument();
  });
  ...
  it("opens a date picker on the day cell and moves the day", async () => {
    ...
    await userEvent.click(await screen.findByRole("button", { name: "01 авг." }));
    await userEvent.click(screen.getByRole("button", { name: "05 августа 2026 г." }));
  ...
  it("sends nothing when the same day is picked again", async () => {
    ...
    await userEvent.click(await screen.findByRole("button", { name: "01 авг." }));
    await userEvent.click(screen.getByRole("button", { name: "01 августа 2026 г." }));
  ...
  it("shows the server's message when the date is taken", async () => {
    ...
    await userEvent.click(await screen.findByRole("button", { name: "01 авг." }));
    await userEvent.click(screen.getByRole("button", { name: "02 августа 2026 г." }));

    const trigger = await screen.findByRole("button", { name: "01 авг." });
    await waitFor(() =>
  ...
  it("asks before deleting a day and then deletes it", async () => {
    ...
    await userEvent.click(await screen.findByRole("button", { name: "Удалить день, 01 авг." }));
    expect(screen.getByText("Удалить день?")).toBeInTheDocument();
    await userEvent.click(screen.getByRole("button", { name: "Удалить" }));

    expect(method).toBe("DELETE");
    await waitFor(() =>
      expect(screen.queryByRole("rowheader", { name: "01 авг." })).not.toBeInTheDocument(),
    );
  });
