...
import { DataTable } from "@/shared/ui/DataTable/DataTable.js";
...
describe("DataTable", () => {
  ...
  it("renders a row action in a trailing cell of every body row", () => {
    render(
      <DataTable
        columns={columns}
        rows={rows}
        rowAction={(row) => <button type="button">Удалить {row.id}</button>}
      />,
    );

    expect(screen.getByRole("button", { name: "Удалить r1" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Удалить r2" })).toBeInTheDocument();
  });
  ...
  it("leaves the footer's action cell empty", () => {
    ...
    expect(screen.queryByRole("button", { name: "Удалить totals" })).not.toBeInTheDocument();
  });
