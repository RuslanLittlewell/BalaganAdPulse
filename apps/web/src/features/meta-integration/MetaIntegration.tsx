...
export function MetaIntegration({ projectId }: { projectId: string }) {
  ...
  const connection = useQuery({
    queryKey,
    queryFn: () => http.get<Connection | null>(path),
    enabled: allowed,
  ...
  const submit = (event: FormEvent) => {
    ...
    setToken("");
    void run(async () => {
      const saved = await http.put<Connection>(path, {
      ...
      <div className="grid gap-1">
        ...
        {connection.data?.lastSuccessAt && (
          <p className="text-xs text-muted-foreground">
            {t("meta.lastSuccess")}{" "}
            {new Date(connection.data.lastSuccessAt).toLocaleString("ru-RU")}
          </p>
        )}
        {connection.data?.lastError && (
          <p role="alert" className="text-xs text-destructive">
            {failureText(connection.data.lastError)}
          </p>
        )}
        {connection.data?.leads?.lastSuccessAt && (
          <p className="text-xs text-muted-foreground">
            {t("meta.leads.lastSuccess")}{" "}
            {new Date(connection.data.leads.lastSuccessAt).toLocaleString(
              "ru-RU",
            )}
          </p>
        )}
        {connection.data?.leads &&
          leadFailureText(connection.data.leads.lastError) && (
            <p role="alert" className="text-xs text-destructive">
              {leadFailureText(connection.data.leads.lastError)}
            </p>
          )}
            ...
            onClick={() =>
              void run(async () => {
                queryClient.setQueryData(
                  queryKey,
                  await http.post<Connection>(`${path}/sync`, {}),
                );
              })
            }
          >
            <RefreshCwIcon
              className={pending || busy ? "animate-spin" : undefined}
            />
            {t("meta.refresh")}
          </Button>
        )}
