...
import { evaluate, type Expression, type ValueResolver } from "./expression.js";
...
export interface StoredValue {
  ...
  readonly numberValue: Decimal | null;
  readonly textValue: string | null;
}

export interface TableRecord {
  readonly id: string;
  /** Already formatted by the adapter, so the domain never touches timezones. */
...
export interface ComputedTable {
  readonly records: ComputedRecord[];
  readonly totals: Record<string, string | null>;
}

function format(value: Decimal | null): string | null {
...
function makeResolver(
  properties: readonly TableProperty[],
  entered: Map<string, Decimal | null>,
): ValueResolver {
...
export function computeTable(
  ...
  const computedRecords = records.map((record) => {
    const entered = new Map<string, Decimal | null>();
    ...
    return { id: record.id, date: record.date, values };
  });

  const aggregated = new Map<string, Decimal | null>();
  for (const property of properties) {
    ...
    // A percentage does not add up: the column total is its average.
