...
async function getRecord(id: string): Promise<CampaignRecord> {
  const record = await prisma.campaignRecord.findUnique({ where: { id } });
...
export async function updateRecord(id: string, input: { date: string }): Promise<CampaignRecord> {
