...
export class PrismaLeadIntakeRepository implements LeadIntakeRepository {
  ...
  async shiftNew(context: TransactionContext, orgId: string, projectId: string, by: number) {
    await this.tx(context).$executeRaw`UPDATE lead SET position = position + ${by} WHERE org_id = ${orgId} AND project_id = ${projectId} AND stage = 'NEW'`;
  }
  ...
  async createImported(context: TransactionContext, input: ImportedLeadInput) {
    ...
    await tx.lead.create({ data: {
      id: input.id, orgId: input.orgId, projectId: input.projectId,
  ...
  async linkAttribution(context: TransactionContext, projectId: string) {
    const tx = this.tx(context);
    const campaigns = await tx.$queryRaw<Array<{ org_id: string }>>`
      ...
      RETURNING lead.org_id`;
    const ads = await tx.$queryRaw<Array<{ org_id: string }>>`
      ...
      RETURNING lead.org_id`;
    const orgIds = new Set([...campaigns, ...ads].map((row) => row.org_id));
    return [...orgIds].map((orgId) => ({ orgId, projectId }));
  }
}
