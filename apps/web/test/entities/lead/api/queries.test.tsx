...
describe("useLeadColumns", () => {
  it("does not refetch a board's columns on a second mount right after the first", async () => {
    let requests = 0;
    server.use(http.get("/api/crm/boards/agency/columns", () => {
      requests += 1;
      return HttpResponse.json([{ id: "col-1", kind: "FIXED", name: "Новый", position: 0 }]);
    }));
    const wrapper = hookWrapper();

    const first = renderHook(() => useLeadColumns("agency"), { wrapper });
    await waitFor(() => expect(first.result.current.isSuccess).toBe(true));
    first.unmount();

    const second = renderHook(() => useLeadColumns("agency"), { wrapper });
